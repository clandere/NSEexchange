load.phi <- function(obs.genome=NULL,init.phi.file=NULL,init.phi.header=FALSE,obs.phi.file=NULL,obs.phi.header=FALSE,norm.expr=FALSE){
  ...
  if(!is.null(obs.phi.file)){
    ...
    if(!is.null(obs.phi2)){
      ...
      phi$obs <- obs.phi2
      dim(phi$obs) <- c(1,length(phi$obs))
      colnames(phi$obs) <- names(obs.phi2)
    }else{
