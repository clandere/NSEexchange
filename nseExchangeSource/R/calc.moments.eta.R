...
calc.moments.eta <- function(gene.list,codon.parms,A_1 = 4,A_2 = 4,B = 0.0025){
  ...
  #        codon.parms - data structure containing codon specific parameters 
  ...
  ELONGATION_PR = codon.parms$elong_pr[sort.list(codon.parms$c_index)]
  ...
  OUTPUT <- .C("calc_eta_moments", as.integer(CODON_INDEX),     AA_COUNT = length(CODON_INDEX),   as.double(PHI),
               as.double(ELONGATION_PR), as.double(MUTATION_RATES),        as.double(1),
