	...
	for (i = 1; i < aa_count; i++) {
		...
		*(delta_eta_second_term_vec) =
		...
		delta_eta_first_term +=
		    (*(NSE_odds_vec++)) * (*(delta_eta_second_term_vec++));

		//(G.A1 + G.A2 i)*NSE_odds_vec[i]*sigma_ratio_vec[i];
	}
	...
	for (i = 1; i < aa_count; i++) {
		...
		for (j = 0; j < jmax; j++) {
			...
			delta_eta_vec[D_index++] =
			    first_term * (1 - z) +
			    second_term * Codon[codon_index].delta_omega[j];
		}

	}
	if (D_index != D_dim) {	//not sure why I need to add one before comparing to D_dim
		error("ERROR in Calc_Delta_Eta_NSE_Vec: %d out of %d indices cycled through. Exiting",
				 D_index, D_dim);
	};
}
